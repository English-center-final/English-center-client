...
function ShortSub(props) {
  const { data, onAnswerSheetClick, image, audio, checkPart, para, name } =
    props;
  const { answers, scrollId, isSubmit, transcript, subPartSelected } =
    ...
    <div className="short_part">
      <Space direction="vertical" style={{ width: '100%' }} size="large">
        <div className={checkPart ? '' : 'content'}>
          ...
          {checkPart ? '' : <div>{parse(audio)}</div>}
        </div>

        {data.map((element, index) => (
          <Space direction="vertical" key={index}>
            <div id={element.stt} className="title_question">
              {element.content} &nbsp;
              {answers[element.stt - 1].selected === '' && isSubmit ? (
                <Text type="warning">
            ...
            {isSubmit ? (
              <Space direction="vertical">
                {renderQuestionAnswer(
                  'a',
                  element.a,
                  answers[element.stt - 1].result,
                  answers[element.stt - 1].selected.toLowerCase()
                )}
                {renderQuestionAnswer(
                  'b',
                  element.b,
              ...
              <Radio.Group
                onChange={(e) => handleSelected(element.stt, e)}
                value={answers[element.stt - 1].selected}
              >
                <Space direction="vertical">
