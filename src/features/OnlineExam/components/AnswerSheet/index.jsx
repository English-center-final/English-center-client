...
function AnswerSheet(props) {
  ...
  const answers_part2 = answers.filter(
    (answer) => answer.stt > 6 && answer.stt <= 31
  );
  const answers_part3 = answers.filter(
    (answer) => answer.stt > 31 && answer.stt <= 70
  );
  const answers_part4 = answers.filter(
    (answer) => answer.stt > 70 && answer.stt <= 100
  );
  const answers_part5 = answers.filter(
    (answer) => answer.stt > 100 && answer.stt <= 130
  );
  const answers_part6 = answers.filter(
    (answer) => answer.stt > 130 && answer.stt <= 146
  );
  const answers_part7 = answers.filter(
    (answer) => answer.stt > 146 && answer.stt <= 200
  );
